<template>
  <q-page class="q-pa-md">
    <div class="text-h5 text-center text-bold q-mb-lg">
      Trouver un garant correspondant à vos besoins
    </div>

    <q-card class="q-pa-lg q-mb-xl bg-grey-1">
      <q-form @submit="handleSearch" class="q-gutter-md">
        ...
        <q-select
          ...
          :options="['Studio', 'T2', 'T3', 'Colocation']"
          outlined
          dense
        />
        <div class="text-right">
          <q-btn label="Lancer la recherche" color="primary" type="submit" unelevated />
        </div>
      </q-form>
    </q-card>

    <!-- Résultats de recherche -->
    <div v-if="garants.length" class="q-gutter-md text-center">
      <div class="text-h6 q-mb-md">Garants potentiels :</div>
    ...
    <div v-else class="text-center text-grey q-mt-lg">Aucun garant trouvé avec ces critères.</div>
  </q-page>
</template>
...
const ville = ref('');
const typeLogement = ref('');
const garants = ref<Garant[]>([]);

const demandesEnvoyees = ref<Garant[]>(JSON.parse(localStorage.getItem('demandes') || '[]'));

// Données simulées
const allGarants: Garant[] = [
...
const handleSearch = () => {
  garants.value = allGarants.filter(
    (garant) =>
      garant.ville.toLowerCase() === ville.value.trim().toLowerCase() &&
      garant.budget >= (budget.value ?? 0),
  );
};
...
function rejeterGarant(id: string) {
  ...
  $q.notify({
    type: 'negative',
    message: 'Garant rejeté.',
  });
}

function accepterGarant(garant: Garant) {
  demandesEnvoyees.value.push(garant);
  localStorage.setItem('demandes', JSON.stringify(demandesEnvoyees.value));
